...
"""Create a portfolio dataframe for series of positions,
allocated against a cash component"""
df1 = df.copy()
df1.drop(['upperband', 'lowerband', 'SMA 20', 's'], axis=1, inplace=True)
df1['pos_diff'] = df1['Position'].diff()
df1['number_of_shares'] = (
    initial_capital - (df1['Closing Price'] * 2)) / df1['Closing Price']
...
df1['holdings'] = df1['Position'] * \
    df1['Closing Price'] * df1['number_of_shares']
df1['cash'] = initial_capital - \
    (df1['pos_diff'] * df1['number_of_shares'] * df1['Closing Price']).cumsum()
df1['cash'].iloc[0] = initial_capital
df1['portfolio_total'] = df1['cash'] + df1['holdings']
return_on_investment = (
    ((-initial_capital + df1['portfolio_total'].iloc[
        -1]) / initial_capital) * 100)


"""computing strategy performance using the cumulative sum of log-returns"""
df1['Market_Log_Return'] = np.log(
    df1['Closing Price'] /
    df1['Closing Price'].shift())
